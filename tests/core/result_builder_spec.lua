...
describe("ResultBuilder", function()
	...
	async.it("builds the results for parameterized with @EmptySource test for gradle", function()
		--given
		local runSpec = {
			cwd = get_current_dir() .. "tests/fixtures/gradle-demo",
			context = {
		...
		local file_path = get_current_dir()
			.. "tests/fixtures/gradle-demo/src/test/java/com/example/EmptySourceTest.java"
		local tree = plugin.discover_positions(file_path)
		...
		local expected = [[
      {
        ["{{current_dir}}tests/fixtures/gradle-demo/src/test/java/com/example/EmptySourceTest.java::emptySourceShouldFail"]
          = {
          errors={{message="emptySourceShouldFail(String)[1] -> org.opentest4j.AssertionFailedError: expected: <false> but was: <true>"}},
          short="emptySourceShouldFail(String)[1] -> org.opentest4j.AssertionFailedError: expected: <false> but was: <true>",
          status="failed"
        ...
        ,
        ["{{current_dir}}tests/fixtures/gradle-demo/src/test/java/com/example/EmptySourceTest.java::emptySourceShouldPass"]
          = {status="passed"}
      }
    ]]
