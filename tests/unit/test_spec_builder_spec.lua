...
describe("SpecBuilder", function()
	...
	it("uses the injected root_finder to resolve the real project root", function()
		local path = Path("/user/home/root/src/test/java/com/example/Test.java")
		local parent_path = path:parent()
		local project_paths = {
			parent_path,
		...
		local find_root_called_with = nil
		local stub_root_finder = {
			find_root = function(dir)
				find_root_called_with = dir
		...
		local spec_builder_instance = SpecBuilder({
			...
			root_finder = stub_root_finder,
		})

		local actual = spec_builder_instance.build_spec({ tree = tree, strategy = "integration" }, config)

		assert(find_root_called_with ~= nil, "the injected root_finder.find_root should have been called")
		---@diagnostic disable-next-line: need-check-nil
		eq(parent_path:to_string(), actual.cwd)
	end)
