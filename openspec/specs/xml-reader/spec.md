# xml-reader Specification

## Purpose
TBD - created by archiving change refactor-xml-reader. Update Purpose after archive.
## Requirements
...
### Requirement: Parse XML file and return full tree

The XML reader MUST expose a `parse(filepath)` method that reads the file, parses the XML, and returns the full parsed tree (or an error) so callers that need to walk the tree themselves (e.g. iterating over arrays of testcases) do not need to call `xml.parse` directly.

#### Scenario: Parse returns the full tree on success

- **WHEN** `parse(filepath)` is called for a valid XML file
- **THEN** the result has `tree` equal to the parsed XML root table and `error = nil`

#### Scenario: Parse surfaces file read errors

- **WHEN** the injected `read_file` throws for the given filepath
- **THEN** the result has `tree = nil` and `error` describing the read failure

#### Scenario: Parse surfaces XML parse errors

- **WHEN** the injected `xml_parse` throws or returns a non-table
- **THEN** the result has `tree = nil` and `error` describing the parse failure
