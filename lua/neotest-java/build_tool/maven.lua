...
maven.get_dependencies_classpath = function()
    -- this can not be cached like that, an autocmd at the very least capturing pom.xml changes should be implemented to know when the classpath might have changed, native jdtls - lsp can provide a hook for classpath change detection, but not for coc...
    ...
    -- at the very least the correct runtime must be set before running maven, this has to be done every time maven is used to work on the project resources
