...
function TestParser.parse_html_gradle_report(filename)
        ...
        for _, row in pairs(table_rows) do
                ...
                local testInformation = { name = name, status = status, displayName = displayName, classname = test_classname }

                if testcases[unique_key] == nil then
                        testcases[unique_key] = { status = "passed", testInformation }
                else
                        table.insert(testcases[unique_key], testInformation)
                end
        end
