...
local function coc_command(id, params, _)
	...
	services = ok_services
			and vim.tbl_filter(function(service)
	...
	if not ok_request or not result then
		log.warn(string.format("Unable to run lsp request %s with payload %s", id, vim.inspect(params)))
	end
...
local function lsp_command(id, params, bufnr)
	...
	if error then
		log.warn(string.format("Unable to run lsp command %s with payload %s", id, vim.inspect(params)))
	end
