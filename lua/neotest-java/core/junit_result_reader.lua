...
---
--- All I/O flows through the injected `XmlReader` (read + parse). Per-file
--- parse errors are logged via the injected `log` and the file is
--- skipped; the rest of the list is still processed. `JunitResult` itself
--- does no I/O — writing test output to disk is `OutputWriter`'s job,
--- used later by `result_builder`.
---
--- Usage:
...
---
--- Tests typically inject a stub `xml_reader` (no real I/O) so the JUnit
--- walk can be exercised without touching the filesystem or async context.
					...
					if suite then
						...
						if tcs then
							...
							for _, tc in ipairs(tcs) do
								table.insert(results, JunitResult:new(tc))
							end
						end
					end
				end
